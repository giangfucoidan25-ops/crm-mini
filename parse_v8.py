...
# Split on lines that start with "Name - DD/MM/YYYY HH:MM"
...
for log in logs:
    ...
    if not log:
        continue
    # Extract date and time from header: "Name - DD/MM/YYYY HH:MM"
    match = re.search(r'^([A-ZÀ-Ỹa-zà-ỹ\s]+) - (\d{2})/(\d{2})/(\d{4})\s+(\d{2}):(\d{2})', log)
    if match:
        day = match.group(2)
        month = match.group(3)
        year = match.group(4)
        ...
        # created_at: ISO format with exact time from the note header
        created_at = f"{year}-{month}-{day}T{hour}:{minute}:00.000Z"
        parsed_logs.append({
            "date": date,
