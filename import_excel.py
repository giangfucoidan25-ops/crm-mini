...
# Load Excel
df = pd.read_excel('Nhập liệu Quản lý Đơn Hàng.xlsx', header=None)
df = df.dropna(how='all')

# Finding the header row and column mapping
header_row_idx = None
col_map = { 'product': 2, 'date': 3, 'phone': 4, 'name': 5, 'status': 6, 'amount': 7 }
note_cols = list(range(8, 17))

for idx, row in df.iterrows():
    row_strs = [str(x).lower().strip() for x in row]
    if any("điện thoại" in x or "sđt" in x for x in row_strs) and any("họ tên" in x or "khách hàng" in x for x in row_strs):
        header_row_idx = idx
        note_cols = []
        for c_idx, val in enumerate(row_strs):
            if "sản phẩm" in val or "s.phẩm" in val:
                col_map['product'] = c_idx
            elif "ngày" in val or "date" in val:
                if 'date' not in col_map or col_map.get('date') == 3:
                    col_map['date'] = c_idx
            elif "điện thoại" in val or "sđt" in val:
                col_map['phone'] = c_idx
            elif "họ tên" in val or "tên" in val or "khách hàng" in val:
                col_map['name'] = c_idx
            elif "trạng thái" in val or "tình trạng" in val:
                col_map['status'] = c_idx
            elif "tiền" in val or "giá" in val or "doanh thu" in val:
                col_map['amount'] = c_idx
            elif "ghi chú" in val or "lần" in val:
                note_cols.append(c_idx)
        if not note_cols:
            note_cols = [i for i in range(len(row_strs)) if i not in col_map.values() and i > col_map['amount']]
        break
...
for idx, row in df.iterrows():
    ...
    phone = str(row[col_map['phone']]).strip() if col_map['phone'] < len(row) else ""
    if col_map['phone'] >= len(row) or pd.isna(row[col_map['phone']]) or not phone or phone.lower() == 'nan':
        continue
    ...
    raw_name_addr = str(row[col_map['name']]).strip() if col_map['name'] < len(row) else ""
    ...
    status_raw = str(row[col_map['status']]).strip().lower() if col_map['status'] < len(row) else ""
    ...
    # Product
    product_name = str(row[col_map['product']]).strip() if col_map['product'] < len(row) else ""
    if product_name.lower() == 'fu8':
        product_name = 'Fu 8'
    if col_map['product'] >= len(row) or pd.isna(row[col_map['product']]) or product_name.lower() == 'nan' or not product_name:
    ...
    # Order Date
    order_date = row[col_map['date']] if col_map['date'] < len(row) else None
    ...
    # Amount
    amount_raw = str(row[col_map['amount']]).replace(',', '').replace('.', '').strip() if col_map['amount'] < len(row) else "0"
    try:
